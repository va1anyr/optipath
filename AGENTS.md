...
off-screen arrow, plus an overview minimap. See the parent `Plugins/Source/AGENTS.md` for the general
...
**PluginBridge: `OptiPather.GetNextMapStep`.** Lets another plugin ask "what map am I routing from to
...
1. `RecomputeMapFinder` (worker) runs `ComputeNextStepGraphInfo` alongside its normal scan — nearest
   unvisited match + the seed (already-owned/in-progress frontier node) the shortest path to it starts
   from (`ReconstructFinderPath`, `path[0]`) — and publishes graph-only data (`Vector2i` coordinate,
   names, hop count) into `nextStepGraphInfo`. The worker never touches `AtlasPanelNode`/`Element` (see
   `GraphNode`'s "never touches live game objects" invariant), so it cannot produce a screen position
   here.
2. `ResolveNextStepScreenPositions` (main thread, called from `Render()` every frame) looks up the seed
   node's live `AtlasNodeDescription.Element.GetClientRect().Center` in this frame's `AtlasPanel
